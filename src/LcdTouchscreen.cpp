...
LcdTouchscreen::LcdTouchscreen(uint8_t cs_pin, SPIClass &spi) : m_ts(cs_pin) {
  m_spi = spi;
}

bool LcdTouchscreen::begin() {
  log_i("begin");
  m_prefs.begin(PARAM_CALIBRATION);
  log_i("begin done");
  return m_ts.begin(m_spi);
}
...
bool LcdTouchscreen::getPoint(int16_t *pX, int16_t *pY, uint16_t *pZ) {
  ...
  if (m_ts.touched()) {
    ...
    x = m_a[0] +
        (((int32_t)m_b[0][0] * pt.x + (int32_t)m_b[0][1] * pt.y) >> 12);
    y = m_a[1] +
        (((int32_t)m_b[1][0] * pt.x + (int32_t)m_b[1][1] * pt.y) >> 12);
...
static void calcLinearRegression(const CalibData *aData, uint8_t nData, int ix,
                                 int iy,
                                 int16_t *pA,  // y = A + B * x;
                                 int16_t *pB, int16_t *pC2) {
  int i, n = nData;
...
bool LcdTouchscreen::setCalibration(const CalibData *aData, uint16_t nData) {
  ...
  for (int i = 0; i < 2; i++) {
    for (int j = 0; j < 2; j++) {
      calcLinearRegression(aData, nData, i, j, &a[i][j], &b[i][j], &c[i][j]);
    }
  }
...
bool LcdTouchscreen::getCalibrationData(uint16_t xs, uint16_t ys,
                                        CalibData *data) {
  int r = 10;
  int16_t xt, yt;
  log_i();
...
bool LcdTouchscreen::calibrate(Adafruit_GFX *dpy, uint16_t fgColor,
  ...
  w = m_dpy->width();
  h = m_dpy->height();
  log_i("touched:%d", touched());
  while (touched()) delay(50);
  log_i();
  return getCalibrationData(m, m, &calibData[0]) &&
...
void LcdTouchscreen::getCalibrationString(char *buf, size_t buf_len) {
  snprintf(buf, buf_len, "%d,%d,%d,%d,%d,%d", m_a[0], m_a[1], m_b[0][0],
           m_b[0][1], m_b[1][0], m_b[1][1]);
}

bool LcdTouchscreen::saveCalibrationData() {
  char buf[80];

  getCalibrationString(buf, sizeof buf);
  m_prefs.putString("data", buf);
  m_prefs.end();
  m_prefs.begin(PARAM_CALIBRATION);
  log_i("put_string data %s", buf);
  return true;
}

bool LcdTouchscreen::loadCalibrationData() {
  char buf[80];

  if (m_prefs.getString("data", buf, sizeof buf) && buf[0]) {
    const char *p = buf;
    char *p0;
    int i, d[6];
